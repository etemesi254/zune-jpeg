...
fn medium_no_sampling_factors() {
    //
    let path = env!("CARGO_MANIFEST_DIR").to_string() + "/tests/inputs/medium_no_samp_2500x1786.jpg";
    let mut decoder = Decoder::new();
    // RGB
    {
        decoder.set_output_colorspace(ColorSpace::RGB);
        let pixels = decoder.decode_file(&path).expect("Test failed decoding");
        write_output("medium_no_samp_rgb_7680_4320.jpg", &pixels,
    ...
    // Grayscale
    {
        ...
        let pixels = decoder.decode_file(&path).expect("Test failed decoding");
        write_output("medium_no_samp_grayscale_7680_4320.jpg", &pixels,
...
fn medium_horizontal_sampling_factors() {
    //
    let path = env!("CARGO_MANIFEST_DIR").to_string() + "/tests/inputs/medium_horiz_samp_2500x1786.jpg";
    let mut decoder = Decoder::new();
    // RGB
    {
        decoder.set_output_colorspace(ColorSpace::RGB);
        let pixels = decoder.decode_file(&path).expect("Test failed decoding");
        write_output("medium_horiz_samp_rgb_7680_4320.jpg", &pixels,
                     decoder.width() as usize, decoder.height() as usize, OutColorSpace::JCS_RGB);
    }
    // Grayscale
    {
        decoder.set_output_colorspace(ColorSpace::GRAYSCALE);
        let pixels = decoder.decode_file(&path).expect("Test failed decoding");
        write_output("medium_horiz_samp_grayscale_7680_4320.jpg", &pixels,
                     decoder.width() as usize, decoder.height() as usize, OutColorSpace::JCS_GRAYSCALE);
    }
    // cymk
    {
        ...
        let pixels = decoder.decode_file(&path).expect("Test failed decoding");
        write_output("medium_horiz_samp_ycbcr_7680_4320.jpg", &pixels,
                     decoder.width() as usize, decoder.height() as usize, OutColorSpace::JCS_YCbCr);
    }
}
