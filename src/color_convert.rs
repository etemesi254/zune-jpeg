...
pub fn ycbcr_to_rgb(
    ...
    // Convert into a slice with 24 elements for Rust to FINALLY SEE WE WON'T GO OUT OF BOUNDS
    let opt: &mut [u8; 24] = v
        .get_mut(0..24)
...
pub fn ycbcr_to_rgba(
    ...
    let (_, v) = output.split_at_mut(*pos);
    // Convert into a slice with 32 elements for Rust to FINALLY SEE WE WON'T GO OUT OF BOUNDS
    let opt: &mut [u8; 32] = v
        .get_mut(0..32)
        .expect("Slice to small cannot write")
        .try_into()
