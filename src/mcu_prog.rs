...
impl Decoder
{
    ...
    ) -> Result<Vec<u8>, DecodeErrors>
    {
        ...
        // size of output image(width*height), larger than normal to give space for
        // uneven images
        ...
        // by unprocessed we mean they haven't been dequantized and inverse DCT carried
        // on them

        // for those pointers storing unprocessed items, zero them out here
        // num_components cannot go above MAX_COMPONENTS(currently all are at a max of
        // 4)
        for (pos, comp) in self.components.iter().enumerate()
        {
            // multiply capacity with sampling factor, it  should be 1*1 for un-sampled
            // images
            self.mcu_block[pos] =
        ...
        // If none of these routines fail,  we can use unsafe in the inner loop without
        // it being UB
        for i in 0..self.input_colorspace.num_components()
        {
            //println!("{},{}",self.components[i].dc_huff_table,self.components[i].
            // ac_huff_table);
            let _ = &self
                .dc_huffman_tables
