...
///  - Do not reallocate a vector created from here reallocation causes it to
///    change its alignment to `mem::align_of::<T>()` which might not be the
///    same as `ALIGN`
///  - Do not use a smaller alignment than a type, please consult
///    `mem::size_of<T>()` for sizes
///  - Do not use this function when drunk, doing so may cause the universe to
///    implode
///
/// # Why?
/// - Most AVX and SSE instructions require memory allocated to a certain byte
///   boundary
/// this allows us to use streaming stores and streaming loads where it makes
/// sense
///
